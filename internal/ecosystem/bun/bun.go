...
import (
	...
	"github.com/packagebeagle/beagle/internal/normalize"
)
...
// file is missing, non-regular, oversized, unreadable, or unparseable so
// callers can leave DirectDependency unset rather than guessing.
//
// The read goes through fsread.Bounded, which enforces the size cap during
// the read itself (io.LimitReader) rather than trusting a stat taken before
// the read — closing the TOCTOU window where a file could grow between a
// stat and an unbounded os.ReadFile.
//
// A non-nil diag is invoked for read or parse failures of a package.json
// that exists. Missing files and non-regular files are silent (no diag):
// the common case is a lockfile checked into a repo without its sibling
// package.json. Too-large files are already diag-warned once by
// fsread.Bounded; loadDirectDeps must not add a second warning for that
// case.
func loadDirectDeps(path string, maxSize int64, diag func(level, path, msg string)) map[string]bool {
	data, err := fsread.Bounded(path, maxSize, diag)
	if err != nil {
		if os.IsNotExist(err) || err.Error() == "not a regular file" || strings.Contains(err.Error(), "exceeds max size") {
			return nil
		}
		if diag != nil {
