...
func TestSplitPnpmLockKey(t *testing.T) {
	cases := []struct {
		...
		{"/@scope/pkg/1.2.3", "@scope/pkg", "1.2.3"},
		{"", "", ""},
		// v5 legacy keys with underscore peer-dependency suffixes (pnpm 6/7;
		// see pnpm discussion #6070). The suffix's '@' must not be mistaken
		// for the v9 name@version separator.
		{"/enzyme-adapter-utils/1.14.0_react@16.13.1", "enzyme-adapter-utils", "1.14.0"},
		{"/@scope/pkg/2.0.0_react@16.13.1", "@scope/pkg", "2.0.0"},
		{"/pkg/1.0.0_@types+react@17.0.2+react@17.0.2", "pkg", "1.0.0"},
		{"/pkg/1.8.9_9181b4d5ce4999e50bf2497382e51d0f", "pkg", "1.8.9"},
		{"/pkg/1.0.0-beta.1_react@16.13.1", "pkg", "1.0.0-beta.1"},
	}
	for _, c := range cases {
