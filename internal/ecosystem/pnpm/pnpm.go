...
func splitPnpmLockKey(key string) (name, version string) {
	...
	// v5 legacy: "name/version[_peer-suffix]". This must run BEFORE the v9
	// '@'-based heuristic below: v5 (pnpm 6/7) encodes resolved peer deps as
	// an underscore suffix on the version, e.g.
	// "enzyme-adapter-utils/1.14.0_react@16.13.1". That suffix's '@' belongs
	// to the peer, not the package, so LastIndexByte(key, '@') would wrongly
	// grab it as the v9 name@version separator (see pnpm discussion #6070).
	// We detect v5 by checking whether the segment after the last '/' starts
	// with a digit (a version) rather than a name (v9 "name@ver" keys with a
	// scoped '/' in them, e.g. "@scope/foo@1.2.3", have a name segment there,
	// not a version, and fall through to the v9 branch as before).
	if i := strings.LastIndexByte(key, '/'); i > 0 {
		seg := key[i+1:]
		if looksLikeVersion(seg) {
			return key[:i], stripPeerSuffix(seg)
		}
	}
...
func (s *Scanner) ScanStorePackageJSON(path, projectPath, name, version string, base model.Record) error {
	if _, err := fsread.Bounded(path, s.MaxFileSize, s.Diag); err != nil {
		return err
	}
	r := base
	r.Ecosystem = Ecosystem
