...
// TestRunReportsTruncatedOnDeadlineExceeded verifies that a scan cut short
// by --max-duration is flagged as truncated (and timed out) on the Result,
// so callers never mistake a partial tree walk for a complete one.
func TestRunReportsTruncatedOnDeadlineExceeded(t *testing.T) {
	root := t.TempDir()
	for i := 0; i < 50; i++ {
		writeFile(t, filepath.Join(root, fmt.Sprintf("proj%d", i), "package-lock.json"),
	...
	if !res.TimedOut {
		t.Errorf("res.TimedOut = false, want true (max-duration exceeded)")
	}
	if !res.Truncated {
		t.Errorf("res.Truncated = false, want true (scan cut short by deadline)")
	}
}

// TestRunReportsTruncatedOnCancellation verifies that a scan interrupted by
// context cancellation (the SIGINT/SIGTERM path in cmd/beagle) is flagged
// as truncated even though it is not a --max-duration timeout: TimedOut
// must stay false so receivers can distinguish the two causes, while
// Truncated must be true so status=complete is never reported.
func TestRunReportsTruncatedOnCancellation(t *testing.T) {
	root := t.TempDir()
	for i := 0; i < 50; i++ {
		writeFile(t, filepath.Join(root, fmt.Sprintf("proj%d", i), "package-lock.json"),
			`{"lockfileVersion":3,"packages":{"node_modules/foo":{"version":"1.0.0"}}}`)
	...
	if res.TimedOut {
		t.Errorf("res.TimedOut = true, want false (this is cancellation, not a max-duration timeout)")
	}
	if !res.Truncated {
