...
// chromiumProfileDirs lists the profile directories directly under a
// Chromium-family browser data dir (e.g. ".../Google/Chrome"). A directory
// is treated as a profile iff it contains a Preferences file — this matches
// "Default", "Profile N", and custom-named profiles alike, and deliberately
// also matches "System Profile"/"Guest Profile"/similar: this is a
// read-only inventory scanner, so over-reporting a root that turns out to
// have no extensions is harmless, while under-reporting silently drops a
// real profile's extensions. If base does not exist or is unreadable, that
// is reported like any other absent root: no profiles, no error escape.
...
func chromiumProfileDirs(base string) []string {
	...
	for _, e := range entries {
		...
		info, err := os.Stat(filepath.Join(base, e.Name(), "Preferences"))
...
func BrowserExtensionCandidateRoots(home string) []string {
	...
	for _, bases := range chromiumBases {
		for _, b := range bases {
			for _, prof := range chromiumProfileDirs(b) {
