...
on disk, what `beagle` reads, and what it deliberately does not.
...
The npm-family wrappers (`npm`, `pnpm`, `yarn`, `bun`) follow the same
rule: a package identity is read only from an *executor* subcommand —
`dlx`, `exec`, `x`, plus the standalone `npx` / `bunx`. Script launchers
(`bun run … start`, `npm start`, bare `yarn dev`) name a `package.json`
script, not a published package, so no identity is inferred and the
record falls back to the server id. Without this, `bun run … start`
records the package `start`, which is itself a real npm package and
would match an exposure catalog on an endpoint that never installed it.
`npm create <name>` and `npm init <name>` are treated the same way: they
do name a published `create-<name>` package, but initializers do not
launch MCP servers, so beagle deliberately does not resolve them.
...
and beagle never executes anything during the scan.
...
Profile coverage on `baseline`: for each Chromium-family browser, every
subdirectory directly under the browser's data dir that contains a
`Preferences` file is treated as a profile — this covers `Default`,
`Profile 1`, `Profile 10`, and custom-named profiles alike (not just a
fixed `Profile 1`..`Profile 9` range), plus the Firefox-family profile
parents. Directories without a `Preferences` file (e.g. `Crashpad`) are
not treated as profiles. The deliberately narrow root list (`Extensions/`
only, one level below each discovered profile) keeps the walker out of
...
- No bundled threat intelligence. Beagle ships no built-in advisory
