# Deploying beagle on macOS

`beagle` is a one-shot binary with no built-in scheduler. On macOS
...
- **LaunchDaemon** (`/Library/LaunchDaemons/`, runs as root). Useful when
  ...
  `beagle` expands the profile's per-user default roots
...
sudo beagle scan \
  ...
  --http-auth bearer \
  --http-token-env BEAGLE_TOKEN \
  --device-id-env BEAGLE_DEVICE_ID
```
...
- **You already run a log shipper** (Vector, Fluent Bit, Filebeat, etc.):
  use `--output file --output-file /var/log/beagle/inventory.ndjson`
...
<dict>
  <key>Label</key>
  <string>com.example.beagle.baseline</string>

  <key>ProgramArguments</key>
  <array>
    <string>/usr/local/bin/beagle</string>
    <string>scan</string>
    ...
    <string>BEAGLE_TOKEN</string>
    <string>--http-gzip</string>
    <string>--device-id-env</string>
    <string>BEAGLE_DEVICE_ID</string>
  </array>

  <key>EnvironmentVariables</key>
  <dict>
    <key>BEAGLE_TOKEN</key>
    <string>__TOKEN__</string>
    <key>BEAGLE_DEVICE_ID</key>
  ...
  <string>/tmp/beagle.baseline.out</string>
  <key>StandardErrorPath</key>
  <string>/var/log/beagle/beagle.baseline.err</string>
</dict>
</plist>
```
...
```xml
<key>Label</key>
<string>com.example.beagle.project</string>
...
<key>ProgramArguments</key>
<array>
  <string>/usr/local/bin/beagle</string>
  ...
  <string>--http-token-env</string><string>BEAGLE_TOKEN</string>
  <string>--http-gzip</string>
  <string>--device-id-env</string><string>BEAGLE_DEVICE_ID</string>
</array>
...
...
/usr/local/bin/beagle scan \
  --profile deep \
  --root "$CONSOLE_HOME" \
  --exposure-catalog /var/db/beagle/campaign-catalog.json \
  --max-duration 10m \
  --output http \
  --http-url https://inventory.example.com/v1/ingest \
  --http-auth bearer \
...
`beagle roots --profile <p>` previews the resolved roots on the
...
set across every real `/Users/<name>/` home. `beagle roots --profile
...
`beagle` reads the device id from an environment variable rather
...
- Grant the `beagle` binary Full Disk Access via MDM (Privacy
...
1. Load the plist: `launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.example.beagle.baseline.plist`
   (LaunchAgent) or `sudo launchctl bootstrap system /Library/LaunchDaemons/...plist`.
2. Trigger one run: `launchctl kickstart -k gui/$(id -u)/com.example.beagle.baseline`.
