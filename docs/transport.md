...
`beagle` writes NDJSON to one of three sinks:
...
```sh
beagle scan --profile deep --root "$HOME" > inventory.ndjson
```
...
beagle scan \
  --profile baseline \
  --output file \
  --output-file /var/log/beagle/inventory.ndjson \
  --append
```
...
- Header: `User-Agent: beagle/<scanner_version>` when available
