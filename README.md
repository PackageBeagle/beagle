<p align="center">
  <img src="docs/logo.png" alt="PackageBeagle logo: a low-poly beagle sniffing a web of package-ecosystem icons" width="340">
</p>

# PackageBeagle

PackageBeagle (binary: `beagle`) is a read-only inventory collector for
package, extension, and developer-tool metadata on macOS and Linux
developer endpoints.
...
Beagle turns that scattered on-disk state into structured NDJSON
...
The same scanner is queryable through osquery: the extension in
[osquery/](osquery/) exposes the inventory as a `beagle_packages` SQL
table for fleets that already run osquery.
...
- Reads only the lockfiles, package-manager install metadata,
  ...
  and credentials in their `env` blocks; Beagle parses these
...
go install github.com/packagebeagle/beagle/cmd/beagle@v0.3.0
```

To build from a checkout:

```sh
go build -o beagle ./cmd/beagle
go test ./...
```
...
```sh
go build -ldflags "-X main.Version=v0.3.0" -o beagle ./cmd/beagle
```
...
```sh
beagle selftest
# selftest OK (5 findings in 1ms)
```

The fixtures live inside the binary, use deliberately fake package
names (`beagle-selftest-evil@0.0.0`), and make no network calls. A
...
Beagle is a one-shot scanner: each invocation performs a single scan
...
For fleets that already run osquery, `beagle.ext` exposes the same
inventory as a `beagle_packages` table. Profile and roots are set per
query, with the same guardrails as the CLI:

```sql
-- curated baseline inventory
SELECT package_name, version, ecosystem FROM beagle_packages;

-- incident sweep of a home dir
...
semantics, and `BEAGLE_*` configuration:
[osquery/README.md](osquery/README.md).
...
PackageBeagle is derived from
[perplexityai/bumblebee](https://github.com/perplexityai/bumblebee)
(Apache License 2.0) and has been substantially modified and renamed.
See [NOTICE](NOTICE).
