...
func TestResolveDeviceIDFromEnv(t *testing.T) {
	t.Setenv("BEAGLE_TEST_DEVICE_ID", "  abc-123  ")
	id, warn := resolveDeviceID("BEAGLE_TEST_DEVICE_ID")
	if id != "abc-123" {
...
func TestResolveDeviceIDMissingEnv(t *testing.T) {
	const name = "BEAGLE_TEST_DEVICE_ID_MISSING"
...
func TestResolveDeviceIDEmptyEnv(t *testing.T) {
	t.Setenv("BEAGLE_TEST_DEVICE_ID_EMPTY", "   ")
	id, warn := resolveDeviceID("BEAGLE_TEST_DEVICE_ID_EMPTY")
	if id != "" {
		t.Fatalf("id = %q, want empty", id)
	}
...
// It returns the absolute parent path (BEAGLE_USERS_DIR value) and
...
func TestResolveRootsBaselineAllUsersExpansion(t *testing.T) {
	...
	t.Setenv("BEAGLE_USERS_DIR", usersDir)
...
func TestVersionStringShape(t *testing.T) {
	s := versionString()
	if !strings.HasPrefix(s, "beagle ") {
		t.Errorf("versionString = %q, want 'beagle ' prefix", s)
	}
...
// TestRunScanReportsPartialStatusOnMaxDurationTruncation verifies the
// scan_summary emitted for a run cut short by --max-duration reports
// status=partial (never complete) and a non-zero exit code, so receivers
// applying the promotion rule in docs/state-model.md never mistake a
// truncated tree walk for a complete snapshot.
func TestRunScanReportsPartialStatusOnMaxDurationTruncation(t *testing.T) {
	root := t.TempDir()
	for i := 0; i < 50; i++ {
		dir := filepath.Join(root, "proj"+string(rune('a'+i%26))+string(rune('0'+i/26)))
		if err := os.MkdirAll(dir, 0o755); err != nil {
			t.Fatal(err)
		}
		body := `{"lockfileVersion":3,"packages":{"node_modules/foo":{"version":"1.0.0"}}}`
	...
	if code == 0 {
		t.Errorf("runScan exit code = 0, want non-zero for a truncated scan")
	}
	...
	for _, line := range lines {
		...
		if summary.RecordType == model.RecordTypeScanSummary {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("no scan_summary record in output: %s", data)
	}
	if summary.Status != model.ScanStatusPartial {
		t.Errorf("scan_summary.status = %q, want %q", summary.Status, model.ScanStatusPartial)
	}
}
